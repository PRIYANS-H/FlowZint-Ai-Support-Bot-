...
def embed_query(query: str) -> list[float]:
    resp = requests.post(EMBED_URL, headers=HEADERS, json={"inputs": query, "options": {"wait_for_model": True}}, timeout=20)
    resp.raise_for_status()
    result = resp.json()
    # HF returns list of lists for batch; single string returns flat list
...
def retrieve_faq(query: str, db: Session) -> dict:
    ...
    try:
        ...
        if row and row.sim >= 0.40:
            # Normalise similarity → confidence (0.40–1.0 → 0.50–0.99)
        ...
        # DB or HF unavailable — caller falls back to JS engine
        raise
