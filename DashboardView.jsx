...
function SentimentSparkline() {
  ...
  useEffect(() => {
    ...
    el._chart = new window.Chart(el, {
      ...
      data: {
        ...
        datasets: [
          {
            label: "Positive", data: HOURLY_SENTIMENT.map(h => h.pos),
            borderColor: "#1D9E75", backgroundColor: "rgba(29,158,117,0.07)",
            fill: true, tension: 0.4, pointRadius: 3, borderWidth: 2,
          },
          {
            label: "Neutral", data: HOURLY_SENTIMENT.map(h => h.neu),
            borderColor: "#534AB7", backgroundColor: "rgba(83,74,183,0.05)",
            fill: true, tension: 0.4, pointRadius: 3, borderWidth: 2,
          },
          {
            label: "Negative", data: HOURLY_SENTIMENT.map(h => h.neg),
  ...
  return (
    <div style={{ position: "relative", height: 160 }}>
      <canvas ref={canvasRef} role="img" aria-label="Hourly sentiment trend today showing positive, neutral, and negative lines">
        Hourly sentiment: positive trend rising from 12 to 28, negative declining from 8 to 3.
      </canvas>
    </div>
  );
}
...
export function DashboardView({ stats, selfCorrected, tickets }) {
  // Derive live churn entries from high-priority tickets for current user
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        {/* Churn risk + recommender — live entries prepended */}
        <Panel>
          <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {allChurnRisks.map((c, i) => (
            <div key={`${c.name}-${i}`} style={{
              ...
              background: c.live ? "rgba(83,74,183,0.03)" : "transparent",
            }}>
